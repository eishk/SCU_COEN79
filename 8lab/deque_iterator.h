...
namespace coen79_lab8
{
    ...
    public:
        ...
        {
            // Uses the != operator
            // Must be already implemented, cannot rely on compiler
            return !(*this!=other);
        ...
        deque_iterator& operator ++( ) // Prefix ++
        {
            ...
            {
                // Move current block pointer up and move boundary to next
            ...
            {
                // Just move cursor forward because not at end of the block's boundary
                ++cursor;
